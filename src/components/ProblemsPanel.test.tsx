// The panel over the shared session, an in-memory tree and canned service replies —
...
const setup = (replies?: Parameters<typeof ports>[0], extra: Partial<React.ComponentProps<typeof Harness>> = {}) => {
  const onOpen = vi.fn<(d: Diagnostic) => Promise<void>>().mockResolvedValue(undefined);
  const utils = render(<Harness p={ports(replies)} onOpen={onOpen} {...extra} />);
  return { onOpen, ...utils };
};
...
describe('ProblemsPanel', () => {
  ...
  it('a failed open is reported, not swallowed', async () => {
    ...
    render(<Harness p={ports(TYPICAL)} onOpen={onOpen} />);
    const list = await screen.findByRole('listbox');
  ...
  it('without a working tree it says so instead of running', () => {
    render(<Harness p={null} onOpen={vi.fn()} />);
